...
victory_img = pygame.image.load('imagens/Icons/victory.png').convert_alpha()
defeat_img = pygame.image.load('imagens/Icons/defeat.png').convert_alpha()
# Imagem da espada
...
class Fighter:
    ...
    def attack(self, target):
        ...
        target.hp -= damage
        # Rodar a animação do inimigo machucado
        target.hurt()
    ...
    def hurt(self):
        # Setar variáveis para animação hurt
        self.action = 2
        self.frame_index = 0
        self.update_time = pygame.time.get_ticks()

    def death(self):
        # Setar variáveis para animação death
        self.action = 3
        self.frame_index = 0
        self.update_time = pygame.time.get_ticks()
...
while run:
    ...
    if game_over == 0:
        ...
        if current_fighter > total_fighters:
            current_fighter = 1
    # Checar se todos os bandidos estão mortos
    alive_bandits = 0 
    for bandit in bandit_list:
        if bandit.alive == True:
            alive_bandits += 1
